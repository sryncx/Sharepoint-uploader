...
<# ############################################################################################################################################################
                                                GUI - Generate Input for Week and Year 
#> ############################################################################################################################################################
...
[string]$Year = $iniFile.Uploader_Modell.Year
[string]$Month = $iniFile.Uploader_Modell.Month
...
While ($I -le 12) {
    ...
    $sourceprefix = "\\UNC-SERVER-Path\Folder\Folder\Folder\$Year\$Month\"
...
<# ############################################################################################################################################################
                                               Upload general Modell/Farb/APL/ZS.PDF - To User
#> ############################################################################################################################################################
UploaderBar-func "Uploader" 40 "Upload User-Files to AD-Leader"
#region loop for user to upload dokuments Modell/Farb/APL/ZS . PDF
$I = 1
While ($I -le 12) {
    $Name = $Vertreter."N$I"
    $GL = $Vertreter."N$I" + " " + $Vertreter."V$I"
    $sourceprefix = "\\UNC-SERVER-Path\Folder\Folder\Folder\$Year\$Month\"
      $sourcepaths = @(
        "$sourceprefix$($Vertreter.D1)$($Dokument.PDF)",
        "$sourceprefix$($Vertreter.D2)$($Dokument.PDF)",
        "$sourceprefix$($Vertreter.D3)$($Dokument.PDF)",
        "$sourceprefix$($Vertreter.D4)$($Dokument.PDF)"
        )
...
#endregion Destination to Upload
#endregion loop for user to upload dokuments Modell/Farb/APL/ZS . PDF
...
$JSONBody = [PSCustomObject][Ordered]@{
    "summary"    = "P$ data"
    "themeColor" = '33cc33'
    "title"      = "Modell/Farben/APL/ZS - Statistik Upload abgeschlossen."
    "text"       = "Monat: $Month
                    Datum: $Date
                    Zeit: $Time"
     }
...
[string]$Subject = "Modell/Farben/APL/ZS - Statistik"
[string]$Body = "Modell/Farben/APL/ZS - Statistik $Month $Year wurde in MS-Teams zu verfügung gestellt."
